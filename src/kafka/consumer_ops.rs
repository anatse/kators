...
impl Debug for ConsumerOps {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_str("ConsumerOps(..)")
    }
}
...
impl ConsumerOps {
    ...
    pub fn read_metadata(&self) -> Result<Metadata> {
        self.consumer
            .fetch_metadata(None, time::Duration::from_millis(1000))
            .map_err(|e| e.into())
    }

    pub fn read_topic_metadata<T: AsRef<str>>(&self, topic: T) -> Result<Metadata> {
        self.consumer
            .fetch_metadata(Some(topic.as_ref()), time::Duration::from_millis(1000))
            .map_err(|e| e.into())
    }

    pub fn read_watermarks<T: AsRef<str>>(&self, topic: T, partition: i32) -> Result<(i64, i64)> {
        self.consumer
            .fetch_watermarks(topic.as_ref(), partition, time::Duration::from_millis(1000))
            .map_err(|e| e.into())
    }

    fn read_from_consumer(
        &self,
        n_last_messages: i64,
    ...
    pub fn read_from_partition_tail<T: AsRef<str>>(
        &self,
        topic: T,
        partition: i32,
        n_last_messages: i64,
    ...
    pub fn read_from_partition_offset<T: AsRef<str>>(
        &self,
        topic: T,
        partition: i32,
        offset: i64,
    ...
    pub fn read_from_partition<T: AsRef<str>>(
        &self,
        topic: T,
        partition: i32,
        max_messages: i64,
